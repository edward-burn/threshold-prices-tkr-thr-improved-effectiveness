...
load("Y:/cprd hes/working data/tkr.to.revision_model.RData")
load("Y:/cprd hes/working data/tkr.to.death_model.RData")


#load("Y:/proms with MI.RData")
load(file = "Y:/PROMs/models/tkr_prom_model.RData")
load(file = "Y:/PROMs/models/tkr_prom_model.bstrap.RData")
...
load(file="Y:/CPRD HES/working data/tkr_cohort.average.characteristics.RData")
tkr.average.characteristics.risks<-tkr_cohort.average.characteristics
...
#clusterExport(cl, "mi.models.tkr_revision")
...
run.model<-function(){
  ...
  get.det.surv.revision<-function(df){
    ...
    det.tps<-summary(mi.models.tkr.to.revision[[1]], #1st MI
                     t = times, 
                     ci=F,
                     newdata = df)
  ...
  # get tkr tps from survival curve
  ...
  # 1) first 10 years
  det.tps.mortality.model<-summary(mi.models.tkr.to.death[[1]],
  ...
  # add patient characteristics to tp tkr
  ...
  sims.revision <- normboot.flexsurvreg(
    mi.models.tkr.to.revision[[1]], 
    B=n.sim, 
    newdata = newdata)
  
  prob.revision<-NULL
  
  #mi.models.tkr_revision[[1]]$call
  #spline
        ...
        prob.revision<-rbind(prob.revision,
                             data.frame(time=using.time,
                                        surv=psurvspline(using.time,  
                                                         gamma = sims.revision, 
                                                         knots = mi.models.tkr.to.revision[[1]]$knots,
                                                         lower.tail = FALSE),
                                        sim=1:n.sim,
                                        group=using.group))}}}
  ...
  sims.death <- normboot.flexsurvreg(
    mi.models.tkr.to.death[[1]], 
    B=n.sim, 
    newdata = newdata)
  
  prob.death<-NULL
  #mi.models.tkr.to.death[[1]]$call
  #spline
  if(length(newdata$group)==1){
      ...
      for(using.group in 1:max(newdata$group)) {
        prob.death<-rbind(prob.death,
                          data.frame(time=using.time,
                                     surv=psurvspline(using.time,  
                                                      gamma = sims.death, 
                                                      knots = mi.models.tkr.to.death[[1]]$knots,
                                                      lower.tail = FALSE),
                                     sim=1:n.sim,
                                     group=using.group))}}}
  ...
  trans<-parLapply(cl, 
                   ...
                   function(df) {
                     ...
                     for(t in 1:number.cycles) {   
                       ...
                       working.tkr_revision.count<-df$p.revision[t] * m.TR[t,1]
                     ...
                     end.tkr_revision<-data.frame(prop.revised=sum(tkr_revision.count)) # sum of transitions to revision
                     end.tkr_revision$group<-df$group[1]
                     end.tkr_revision$sim<-df$sim[1]
                     end.tkr_revision$revision.reduction<-df$revision.reduction[1]
                     ...
                     output <- list("markov.trace" = m.TR, 
                                    "proportion.tkr_revision" = end.tkr_revision)
                     output
                   })
  ...
  for (i in 1:length(trans)) {
    proportion.tkr_revision[[i]]<-trans[[i]]$proportion.tkr_revision
  ...
  proportion.tkr_revision<-parLapply(cl,proportion.tkr_revision, 
                                     get.characteristics)
  ...
  markov.trace<-markov.trace %>% 
    mutate(tkr_revision.qol=(((((q1_eq5d+pred.qol.unrevised)/2)
                               *6)+
  ...
  # will also make tps and proportion.tkr_revision
  # dataframes
  tps<-plyr::ldply(tps, 
                   data.frame, .id=NULL)
  # tps<<-tps
  proportion.tkr_revision<-plyr::ldply(proportion.tkr_revision, 
                                       data.frame, .id=NULL)
  #  proportion.tkr_revision<<-proportion.tkr_revision
  print(Sys.time()-start)
  ...
  pred.cost<-lapply(pred.cost,
                    function(df) {
                      ...
                      if(df$sim[1]!="deterministic"){
                        df$tkr.cost<-
                          exp(predict(tkr.cost.model.bstrap.mi_1[[as.numeric(df$sim)]],
                                      df %>% 
                                        mutate(diagnosis=
                                                 ifelse(diagnosis=="k_ost",
                                                        "OA", "RA"),
                      ...
                      df$tkr_revision.cost<-NA
                      if(df$sim=="deterministic"){
                        df$tkr_revision.cost<-exp(predict(glm.tkr_revision.reference_cost,
                                                          df %>% 
                                                            mutate(diagnosis=
                                                                     ifelse(diagnosis=="k_ost",
                                                                            "OA", "RA"),
                      ...
                      if(df$sim!="deterministic"){
                        df$tkr_revision.cost<-exp(predict(tkr_revision.cost.model.bstrap[[as.numeric(df$sim)]],
                                                          df %>% 
                                                            mutate(diagnosis=
                                                                     ifelse(diagnosis=="k_ost",
                                                                            "OA", "RA"),
                                                                   tkr_revision.1.age=working.age, #dependent on time in model
                                                                   gender= gender,#"Male",
                                                                   tkr_revision.1.RCS.charlson.ra.omitted= RCS ,#"0",
                                                                   IMD_2004_quintiles= IMD ,#"2",
                                                                   tkr_revision.1.BMI=BMI,#30,
  ...
  pred.cost<-plyr::ldply(pred.cost, 
                         data.frame, .id=NULL) %>% 
    select(group, sim, time, tkr.cost, tkr_revision.cost)
  ...
  # tkr cost- incurred by all only at time 1
  head(markov.trace$tkr.cost)
  markov.trace<-markov.trace %>% 
    mutate(tkr.cost=ifelse(time==1, tkr.cost, NA))
  
  
  # tkr_revision cost 
  # multiply by number progressing to revision state
  markov.trace$tkr_revision.cost<-markov.trace$tkr_revision.cost*markov.trace$tkr_revision
  ...
  # missing is zero
  markov.trace<-markov.trace %>% 
    mutate(cost=rowSums(cbind(tkr.cost,tkr_revision.cost), na.rm=TRUE)) 
  #discounted cost
  markov.trace<-markov.trace %>% 
...
characteristics<-
  rbind(
    expand.grid(age=median_tkr_age,
                gender=mode_tkr_gender,
                diagnosis=mode_tkr_diagnosis,
                IMD=as.character(mode_tkr_IMD_2004_quintiles),
                RCS=mode_tkr_RCS,
                charlson=tkr.average.characteristics.risks$mode_charlson,
                BMI=median_tkr_BMI,
                smoke=mode_tkr_smoke,
                smoking_status=tkr.average.characteristics.risks$mode_smoking_status,
                q1_eq5d=median_tkr_q1_eq5d,
                year=median_tkr_year))

characteristics<-characteristics%>% 
  mutate(age.centered=age-median_tkr_age,
         BMI.centered=BMI-median_tkr_BMI)

#tkr.average.characteristics.risks$mode_smoking_status
...
# save -----
save("tps",
     file="C:/Users/Ed/Dropbox/DPhil data cprd hes analysis/threshold prices tkr thr improved effectiveness/model output/tkr.average.characteristics.tps.RData")
save("markov.trace",
     file="C:/Users/Ed/Dropbox/DPhil data cprd hes analysis/threshold prices tkr thr improved effectiveness/model output/tkr.average.characteristics.markov.trace.RData")
save("QALYs.costs",
...
# save -----
save("tps",
     file="C:/Users/Ed/Dropbox/DPhil data cprd hes analysis/threshold prices tkr thr improved effectiveness/model output/tkr.age.gender.tps.RData")
save("markov.trace",
     file="C:/Users/Ed/Dropbox/DPhil data cprd hes analysis/threshold prices tkr thr improved effectiveness/model output/tkr.age.gender.markov.trace.RData")
save("QALYs.costs",
